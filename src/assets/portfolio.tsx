...
const Portfolio: React.FC = () => {
  ...
  /* ✅ CRITICAL FIX: sync active image */
  useEffect(() => {
    if (selectedProject) {
  ...
  return (
    <section
      id="projects"
      className="relative min-h-screen bg-background text-foreground py-24 overflow-x-hidden"
    >
                  ...
                  <div className="flex flex-wrap space-x-3.5">
                    {selectedProject.gallery.map((img, idx) => (
                      <div
                        ...
                        className={`h-32 w-32 relative bg-zinc-100 border rounded-lg overflow-hidden cursor-pointer transition-all shadow-sm ${activeImage === img ? "ring-2 ring-indigo-600" : "hover:ring-2 ring-zinc-900"}`}
                      >
                        <Image
                          src={img}
                          alt={`Gallery ${idx}`}
